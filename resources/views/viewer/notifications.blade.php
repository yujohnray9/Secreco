...
<div class="page active" id="page-notifications">
  <div class="pg-banner">
    <div>
      ...
      <div class="pg-banner-sub">System updates, publications, and consortium announcements</div>
    </div>
  </div>
  ...
  <div class="fc-card">
    ...
    <div class="fc-card-body">
      <div id="vNotifContainer" class="notif-list">
...
document.addEventListener('DOMContentLoaded', async function () {
  function getViewerNotifSvg(type, notifType, icon) {
    const t = ((notifType || '') + ' ' + (type || '')).toLowerCase();
    const ic = (icon || '').toLowerCase();

    if (t.includes('edit') || t.includes('update') || ic.includes('edit') || ic === '✏️' || ic === '📝') {
    ...
    if (t.includes('corr') || t.includes('return') || t === 'red' || ic === '🔴' || ic === '↩️') {
    ...
    if (t.includes('accept') || t.includes('approv') || t === 'green' || ic === '✅') {
  ...
  function getViewerColorClass(type, notifType) {
    const t = ((notifType || '') + ' ' + (type || '')).toLowerCase();
    if (t.includes('corr') || t.includes('return') || t.includes('red') || t.includes('delete')) return 'red';
    if (t.includes('accept') || t.includes('green')) return 'green';
    if (t.includes('user') || t.includes('yellow')) return 'yellow';
    return 'blue';
  }
      ...
      if (json.notifications.length === 0) {
        container.innerHTML = `
          <div style="padding:48px 24px;text-align:center;color:#9ca3af">
            ...
            <div style="font-size:12px;color:#9ca3af;margin-top:3px">You're all caught up with system updates.</div>
          </div>`;
      } else {
        container.innerHTML = json.notifications.map(n => {
          const colorClass = getViewerColorClass(n.type, n.notif_type);
          const svgIcon = getViewerNotifSvg(n.type, n.notif_type, n.icon);
          return `
