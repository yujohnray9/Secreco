...
(function () {
  ...
  ───────────────────────────────────────── */
  const SECTIONS = [
    { label: 'R&D Mgt. & Coord.', tables: ['T1','T2a','T2b','T3','T4','T5','T6','T7a','T7b'] },
    { label: 'Strategic R&D',     tables: ['T8a','T8b','T9'] },
    { label: 'Results Utilization', tables: ['T10','T11','T12','T13'] },
    { label: 'Capability & Gov.', tables: ['T14','T15','T16','T17','T18','T19'] },
    { label: 'Policy Analysis',   tables: ['T20a','T20b'] },
  ];

  const TABLE_TITLES = {
    T1:'AIHRs', T2a:'RSRDH Summary', T2b:'RSRDH Participants',
    T3:'Projects Monitored', T4:'Resources Shared', T5:'Resources Generated',
    T6:'Linkages', T7a:'Databases', T7b:'Info Systems',
    T8a:'R&D Programs', T8b:'Collaborative R&D', T9:'Technologies from R&D',
    T10:'TT Programs', T11:'Technologies Extended', T12:'Commercialized',
    T13:'Promotion Approaches', T14:'Non-degree Trainings',
    T15:'Equipment/Facilities', T16:'Awards', T17:'Regular Meetings',
    T18:'CMI Contributions', T19:'New Initiatives',
    T20a:'Policy Research', T20b:'Policies',
  };
  ...
  const STATUS_ICON = {
    ...
    'not-started': '<svg viewBox="0 0 24 24" width="13" height="13" fill="none" stroke="#9ca3af" stroke-width="2"><circle cx="12" cy="12" r="10"/></svg>',
  };
  ...
  let _submittedTables = []; // tables that were 'done' in the submitted snapshot
  ...
  function renderFillNav() {
    ...
    nav.innerHTML = SECTIONS.map(s => {
      const items = s.tables.map(no => {
        const st     = _status[no] || 'not-started';
        const ttl    = TABLE_TITLES[no] || no;
        const active = (window._cmiActiveTable === no) ? ' active' : '';
        return `<div class="fill-nav-sub ${st}${active}" onclick="CMI.showTable('${no}')">`
             + `${STATUS_ICON[st]} ${no} — ${ttl}</div>`;
      }).join('');

      return `<div class="fill-nav-item"><svg viewBox="0 0 24 24" width="13" height="13" fill="none" stroke="currentColor" stroke-width="2" style="margin-right:4px"><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"/></svg>${s.label}</div>${items}`;
    }).join('');
  }
  ...
  CMI.showTable = function (no) {
    ...
    if (!def) {
      body.innerHTML = `<div style="padding:32px;color:var(--text-muted);font-size:14px">`
                     + `⚙️ Table <strong>${no}</strong> is not yet implemented.</div>`;
    ...
    // Lock only if this specific table was 'done' in the submitted snapshot
  ...
  function applyLock(tableNo) {
    ...
    // Only lock if this table is in the submitted snapshot
    ...
    // Hide action buttons (Save, Documentation, Add Row, Remove Row — all use onclick)
    body.querySelectorAll('button[onclick]').forEach(el => {
      el.style.display = 'none';
    });

    // Inject submitted banner if not yet there
  ...
  /* ─────────────────────────────────────────
     LOCK WATCHDOG
     Most tables load their rows asynchronously (fetch inside loadData()),
     which finishes AFTER showTable()'s initial applyLock() call — so any
     inputs added later (the actual data rows) would slip past the lock.
     This observer re-applies the lock whenever new elements are added
     to #fillBody while the active table is a locked/submitted one.
  ───────────────────────────────────────── */
  ...
  /* ─────────────────────────────────────────
     ...
     Replaces browser confirm() — no "localhost says" header.
     Usage: cmiConfirm({ title, html, okLabel, onOk })
  ───────────────────────────────────────── */
  ...
  CMI.submitReport = function () {
    ...
    const blank  = allNos.filter(n => !_status[n] || _status[n] === 'not-started').length;
    const total  = allNos.length;
    const yr     = new Date().getFullYear();

    const pct = Math.round((done / total) * 100);
    const bar = `<div style="background:#e8f5e9;border-radius:6px;height:8px;margin:10px 0 4px;overflow:hidden">
      <div style="width:${pct}%;height:100%;background:#2e7d32;border-radius:6px;transition:width .4s"></div></div>
      <div style="font-size:11px;color:#666;margin-bottom:12px">${pct}% complete</div>`;

    const rows = [
      ['✅', 'Complete', done, '#2e7d32'],
      ['⏳', 'Draft',    draft, '#e65100'],
      ['⚪', 'Blank',    blank, '#888'],
        ...
        fetch(API_SUBMIT, { method: 'POST' })
          ...
          .then(res => {
            ...
            if (res.success) {
              ...
              // Update submitted tables — lock only the 'done' ones
              _isSubmitted     = true;
              _submittedAt     = res.submitted_at || new Date().toISOString();
              _submittedTables = allNos.filter(n => _status[n] === 'done');
              CMI.lockReport();
            } else {
