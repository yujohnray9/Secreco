...
function renderT13(data) {
  ...
  data.forEach(cmi => {
    ...
    const rows = cmi.rows || [];
    const touched = rows.filter(r => (r.remarks || '').trim() !== '').length;
    ...
    out += `
      <div class="rpt-cmi-block" style="margin-bottom:18px">
        <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;margin-bottom:6px">
          ...
          ${badge}
          <span style="font-size:11px;color:var(--text-muted)">${touched} of ${rows.length || 6} with remarks · Last updated: ${esc(cmi.updated_at ?? '—')}</span>
        </div>
        ${docsHtml || ''}
        ${rows.length ? `
